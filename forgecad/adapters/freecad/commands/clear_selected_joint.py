...
def _joint_member_ids(document, node_xyz):
    """Return straight member IDs touching a joint at an end or in the interior."""
    result = []
    for obj in frame_member_objects(document):
        ident = _layout_id(obj)
        if not ident or not hasattr(obj, "StartPoint") or not hasattr(obj, "EndPoint"):
            continue
        if point_on_segment(node_xyz, _xyz(obj.StartPoint), _xyz(obj.EndPoint)):
            if ident not in result:
                result.append(ident)
    return tuple(result)


def apply_clear_selected_joint(document, selection):
    node_xyz, selected_ids = selected_clear_request(selection)
    position = FreeCAD.Vector(*node_xyz)
    node_key_value = vector_key(position)
    saved = load_joint_treatment(document, node_key_value)
    existing_copes = load_joint_cope_pairs(document, node_key_value)
    existing_throughs = load_joint_through_pairs(document, node_key_value)
    (
        clear_primary,
        remaining_copes,
        removed_copes,
        ...
        existing_copes,
        selected_ids,
        _joint_member_ids(document, node_xyz),
        existing_throughs,
    )

    started = False
    try:
        document.openTransaction("Clear Selected ForgeCAD Joint Treatment")
        started = True
        if clear_primary:
            save_joint_treatment(document, node_key_value, JointTreatmentMode.AUTO, ())
        if removed_copes:
            replace_joint_cope_pairs(document, node_key_value, remaining_copes)
        if removed_throughs:
            replace_joint_through_pairs(document, node_key_value, remaining_throughs)
        ...
        raise
    return clear_primary, removed_copes, removed_throughs
